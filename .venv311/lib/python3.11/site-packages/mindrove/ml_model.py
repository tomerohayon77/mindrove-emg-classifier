...
from mindrove.board_shim import MindRoveError, LogLevels
from mindrove.exit_codes import MindRoveExitCodes
from numpy.ctypeslib import ndpointer


class MindRoveMetrics(enum.IntEnum):
...
class MindRoveClassifiers(enum.IntEnum):
...
class MindRoveModelParams(object):
    ...
    def __init__(self):
        if platform.system() == 'Windows':
            ...
            if arch == 'armv7l':  # ARM 32-bit
                dll_path = 'lib/arm32/libMLModule.so'
            elif arch == 'aarch64':  # ARM 64-bit
                dll_path = 'lib/aarch64/libMLModule.so'
...
class MLModel(object):
    ...
    :type model_params: MindRoveModelParams
    """

    def __init__(self, model_params: MindRoveModelParams) -> None:
    ...
    def release_all(cls) -> None:
        ...
        if res != MindRoveExitCodes.STATUS_OK.value:
            raise MindRoveError('unable to release classifiers', res)
    ...
    def prepare(self) -> None:
        ...
        if res != MindRoveExitCodes.STATUS_OK.value:
            raise MindRoveError('unable to prepare classifier', res)
    ...
    def release(self) -> None:
        ...
        if res != MindRoveExitCodes.STATUS_OK.value:
            raise MindRoveError('unable to release classifier', res)
    ...
    def predict(self, data) -> List:
        ...
        if res != MindRoveExitCodes.STATUS_OK.value:
            raise MindRoveError('unable to calc metric', res)
