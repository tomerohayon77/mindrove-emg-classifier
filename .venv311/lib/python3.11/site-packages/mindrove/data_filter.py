...
class DataHandlerDLL(object):
    ...
    def __init__(self):
        ...
        self.get_rmssd_hrv = self.lib.get_rmssd_hrv
        self.get_rmssd_hrv.restype = ctypes.c_int
        self.get_rmssd_hrv.argtypes = [
            ndpointer(ctypes.c_double),
            ndpointer(ctypes.c_double),
            ndpointer(ctypes.c_double),
            ctypes.c_int,
...
class DataFilter(object):
    ...
    @classmethod
    def get_heart_rate(cls, ppg_ir, ppg_red, ppg_green, sampling_rate: int, fft_size: int):
        """get heart rate
        ...
        :param ppg_red: input array
        :type ppg_red: NDArray[Float64]
        :param ppg_green: input array
        :type ppg_green: NDArray[Float64]
        :param sampling_rate: sampling rate
        :type sampling_rate: int
        ...
        check_memory_layout_row_major(ppg_ir, 1)
        check_memory_layout_row_major(ppg_red, 1)
        check_memory_layout_row_major(ppg_green, 1)
        if ppg_ir.shape[0] != ppg_red.shape[0] != ppg_green.shape[0]:
            raise MindRoveError('invalid shapes', MindRoveExitCodes.INVALID_ARGUMENTS_ERROR)
        output = numpy.zeros(1).astype(numpy.float64)
        res = DataHandlerDLL.get_instance().get_heart_rate(ppg_ir, ppg_red, ppg_green, ppg_red.shape[0], sampling_rate,
                                                           fft_size, output)
        if res != MindRoveExitCodes.STATUS_OK.value:
    ...
    @classmethod
    def get_rmssd_hrv(cls, ppg_ir, ppg_red, ppg_green, sampling_rate: int):
        """get heart rate
        ...
        :param sampling_rate: sampling rate
        :type sampling_rate: int
        :return: heart rate variability (RMSSD)
        :rtype: float
        """
        check_memory_layout_row_major(ppg_ir, 1)
        check_memory_layout_row_major(ppg_red, 1)
        ...
        if ppg_ir.shape[0] != ppg_red.shape[0] != ppg_green.shape[0]:
            raise MindRoveError('invalid shapes', MindRoveExitCodes.INVALID_ARGUMENTS_ERROR)
        output = numpy.zeros(1).astype(numpy.float64)
        res = DataHandlerDLL.get_instance().get_rmssd_hrv(ppg_ir, ppg_red, ppg_green, ppg_red.shape[0], sampling_rate,
                                                        output)
        if res != MindRoveExitCodes.STATUS_OK.value:
            raise MindRoveError('unable to calc heart rate variablity', res)
        return output[0]
