...
class BoardControllerDLL(object):
    ...
    def __init__(self):
        ...
        self.get_ppg_raw_channels = self.lib.get_ppg_raw_channels
        self.get_ppg_raw_channels.restype = ctypes.c_int
        self.get_ppg_raw_channels.argtypes = [
            ctypes.c_int,
            ctypes.c_int,
            ndpointer(ctypes.c_int32),
            ndpointer(ctypes.c_int32)
...
class BoardShim(object):
    ...
    @classmethod
    def get_ppg_raw_channels(cls, board_id: int, preset: int = MindRovePresets.DEFAULT_PRESET) -> List[int]:
        """get list of ppg channels in resulting data table for a board
        ...
        res = BoardControllerDLL.get_instance().get_ppg_raw_channels(board_id, preset, ppg_channels, num_channels)
        if res != MindRoveExitCodes.STATUS_OK.value:
            raise MindRoveError('unable to request info about this board', res)
        result = ppg_channels.tolist()[0:num_channels[0]]
        return result
